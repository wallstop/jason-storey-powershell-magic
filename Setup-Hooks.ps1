...
function Install-PreCommitFramework {
    ...
    try {
        # Install pre-commit
        Write-Info "Installing pre-commit via pip (using $pythonCmd)..."
...
function Install-GitHooks {
    ...
    echo 'âŒ No PowerShell found and no bash hook available'
    exit 1
    fi
    fi
"@
...
function Get-BestMethod {
    if (Test-PreCommitFramework) {
        ...
        return 'precommit'
    } elseif ($null -ne (Test-PythonAvailable)) {
...
function Main {
    ...
    switch ($installMethod) {
        'precommit' {
            if (Test-PreCommitFramework) {
                # Pre-commit already installed, just run install to set up hooks
                $success = Install-PreCommitFramework
            } else {
                # Try to install pre-commit framework
                $success = Install-PreCommitFramework
                if (-not $success) {
