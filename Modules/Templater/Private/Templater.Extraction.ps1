...
function Invoke-TemplateExtraction {
    ...
    switch ($Template.Type) {
        'File' {
            # Extract archive
            $extractResult = Extract-Archive -ArchivePath $Template.Path -DestinationPath $DestinationPath -Force:$Force
            if ($extractResult) {
        ...
        'Folder' {
            ...
            Write-Host "Template copied successfully! ($($copyResult.CopiedFiles) files, $($copyResult.DirectoryCount) directories)" -ForegroundColor Green
        }
    }
